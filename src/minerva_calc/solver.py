...
class Solver:
    ...
    def combine_letters(self, inp: list[str]) -> list[str]:
        new_inp = inp
    ...
    def _print_tokens(self) -> None:
        str_tokens = [f"Index: {index} | Type: {type_to_str(element.type)} | Value: {element.value}"\
                        for index, element in enumerate(self.tokens)]
        print(str_tokens)
